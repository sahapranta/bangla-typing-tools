...
PopupVirtualKeyboard = new function() {
    ...
    self.show = function (target) {
        if (!hWnd || hWnd.closed) {
          hWnd = (window.showModelessDialog||window.open)(p+"vk_popup.html",window.showModelessDialog?window:"_blank","status=0,title=0,dependent=yes,resizable=no,scrollbars=no,width=500,top=300,left=400,height=500");
          tgt = target;
          return true;
        }
        return false;
